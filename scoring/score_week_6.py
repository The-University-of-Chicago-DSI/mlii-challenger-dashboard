...
SUBMISSION_PATTERN = "*Week_6_y_pred_*.*"
DEFAULT_TARGET_FILENAME = "Week_6_y_test.npy"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description=f"Score Week 6 challenge submissions.")
    add_common_cli_args(parser)
    parser.add_argument("--scores-output", type=Path, default=Path(f"week_6_scores.csv"))
    parser.add_argument("--leaderboard-output", type=Path, default=Path(f"week_6_leaderboard_rows.csv"))
    args = parser.parse_args()

    submissions_dir = args.submissions_dir or (base_dir / "submissions" / f"week_6")
